...
async def test_vehicle_db_count_flow():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.delete(API_DELETE_URL, headers=headers, timeout=60.0)
        assert response.status_code == 200

        # Step 2: build models
        response = await client.get(API_BUILD_URL, headers=headers, timeout=60.0)
        assert response.status_code == 200
        data = response.json()
        ...
        with open("services/python-services/tests/test1.jpg", "rb") as f:
            response = await client.post(API_URL, headers=headers, files={"file1": ("test1.jpg", f, "image/jpeg")}, timeout=60.0)
            assert response.status_code == 200

        time.sleep(3)

        # Check that there are 3 vehicles in the database
        response = await client.get(API_GET_URL, headers=headers)
        assert response.status_code == 200
        data = response.json()
